...
<span style="float:right; margin:0;" class="help-block">
    ...
    {{__('WordCount')}}&nbsp;<span class="count">0</span> / {{intval($pairs['maxlength'])}}
</span>
<script>
    $(function(){
        var input = $("[name='{{$pairs['name']}}']");
        var countLbl = $(input).closest('[class*="col"]').find('[class="count"]');
        if (countLbl.length == 0) {
            countLbl = $(input).closest('[class*="form-group"]').find('[class="count"]');
        }
        setTimeout(function(){
            if (countLbl) {
