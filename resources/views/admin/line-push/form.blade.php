...
@section('content')
            ...
            <div class="box box-default">
                <div class="box-header with-border">
                    ...
                    <div class="form-group">
                        ...
                        <select id="target" name="targets[]" class="form-control" style="display:inline; width:300px;" multiple="multiple">
                            @foreach ( $push_target as $k => $v )
                                @if(isset($targets) && in_array($k, $targets))
                    ...
                    <div class="form-group">
                        <label for="send_at">預定發送</label>
                        <input type="text" name="send_at" id="send_at" class="form-control inputmask" data-inputmask="'mask': '9999/99/99 99:99'" style="display:inline; width:150px;">
                    </div>
                    <label style="color:red;">圖片寬度：1280px 以上</label>
                    <div class="btn-group pull-right">
                        <div class="btn-group pull-right" style="margin-right: 10px">
                            <a href="{{route('line-push.index')}}" class="btn btn-sm btn-default"><i class="fa fa-list"></i>&nbsp;{{__('admin.list')}}</a>
                        </div>
                    </div>
                </div>
    ...
    <script>
        //
        $(function(){
            $("#target").select2({"allowClear":true,"placeholder":"\u767c\u9001\u5c0d\u8c61"});
            $(".inputmask").inputmask();
        ...
        async('{{ admin_asset("/vendor/libs/ImageMapEditor/imgmap.js") }}', function(){
            async('{{ admin_asset("/vendor/libs/ImageMapEditor/line-rich-menu.js") }}', function() {
                ...
                $(function(){
                    ...
                    <?php $newtimestamp = strtotime(date('Y/m/d H:i').' + 5 minute'); ?>
                    $("#send_at").val("{{date('Y-m-d H:i:s', $newtimestamp)}}");
                    if("{{$map['image']}}") {
                        $("#title").val("{{$map['title']}}");
                        $("#target").val("{{$map['target']}}");
                        $("#send_at").val("{{$map['send_at']}}");
