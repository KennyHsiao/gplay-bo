 ...
 * Modify By Hsuan-ming Yang <yang5664@gmail.com>
 * If you'r using with commercial please contact me.
 */
;(function ($, window, document, undefined) {
  ...
  var defaults = {
      ...
      onClickReadOnlyPeriod: function () {},
  ...
  function Plugin(element, options) {
    ...
    this.periodOptions = {
      quota: this.settings.periodQuota,
  ...
  $.extend(Plugin.prototype, {
     ...
     * 取得 星期幾
     */
    dateOfWeekDay: function(year, month, day) {
      return new Date(year, month-1, day).getDay();
    },

    calendar: function(args) {
      var year = args[1]['year'],
          month = args[1]['month'],
          startTime = args[1]['startTime'],
          endTime = args[1]['endTime'];
      this.settings.startTime = startTime;
    ...
    init: function (year, month) {
      ...
      this.periodPosition = 122 / this.periodInterval; // 格子高度
    ...
    create: function (year, month) {
      ...
      for (var j = this.settings.startTime; j < this.settings.endTime + 1; j++) {
        jqsDayHeight += 117.2;
      ...
      // .jqs-day 高度
      $('.jqs-day').css('height', jqsDayHeight+'px');
      ...
      for (var k = 0; k < monthDays; k++) {
        var dayOfWeek = this.dateOfWeekDay(year, month, k);
        $('<div class="jqs-grid-day">'+ month +'/'+ (k+1) + '<br>' + this.settings.daysList[dayOfWeek] + dayRemove + dayDuplicate + '</div>').
     ...
     * @param options
     */
    add: function (parent, position, height, options, uuid) {
      ...
      var periodQuota = '<div class="jqs-period-quota">' + options.quota + '</div>';
      var periodOrders = options.orders > 0 ? '<div class="jqs-period-orders">' + options.orders + '</div>' : '';
      var periodTime = '<div class="jqs-period-time">' + this.periodInit(position, position + height) + '</div>';
      var period = $('<div class="jqs-period '+(options.orders > 0 ? "jqs-orders" : "")+' ">' +
        '<div class="jqs-period-container" data-uuid="'+uuid+'">' + periodTime + periodQuota + periodOrders + periodRemove + periodDuplicate + '</div>' +
        '</div>').css({
        'top': position * this.periodPosition,
        'height': height * this.periodPosition
      }).attr('id', this.uniqId()).attr('quota', options.quota).appendTo(parent);
      ...
      if (this.settings.mode === 'read') {
        ...
        if (this.settings.periodOptions) {
          period.click(function (event) {
            $this.settings.onClickReadOnlyPeriod.call(this, event, period, $this.element);
          });
        }
      }
    ...
    remove: function (period) {
      if (!this.settings.onRemovePeriod.call(this, period, this.periodData(period))) {
        period.remove();
        this.closeOptions();
      }
    },
    ...
    /**
     * Change(resize / drag) a period
     * @param period
     */
    change: function (period) {
      if (!this.settings.onChangePeriod.call(this, period, this.periodData(period))) {
        // do nothing
      }
    },
    ...
    openOptions: function (event, period) {
      ...
      // quota
      var quota = $('.jqs-period-quota', period).text();
      var quotaInput = '<div class="jqs-options-title-container">' +
        '<input type="number" min=1 placeholder="' + this.settings.periodQuotaPlaceholder +
        '" value="' + quota + '" class="jqs-options-quota"></div>';
      ...
      $('<div class="jqs-options">' + time + quotaInput + colorInput + remove + duplicate + close + '</div>').css({
        top: top,
        left: left
      ...
      $('.jqs-options-quota', this.element).keyup(function () {
        $('.jqs-period-quota', period).text($(this).val());
        period.attr('quota', $(this).val());
      });
    ...
    periodText: function (period) {
      ...
      $('.jqs-period-quota', period).height(20);
      $('.jqs-period-orders', period).height(20);
    },
    ...
    periodData: function (_period) {
      ...
      var _uuid = $('.jqs-period-container', period).data('uuid') == 'undefined' ? '' : $('.jqs-period-container', period).data('uuid');
      return {
        uuid: _uuid,
        start: this.periodFormat(start),
        end: this.periodFormat(end),
        quota: $('.jqs-period-quota', period).text(),
    ...
    periodFormat: function (position) {
      ...
      var hour = Math.floor(position / this.periodInterval) + this.settings.startTime;
      var mn = (position / this.periodInterval - (hour - this.settings.startTime)) * 60;
    ...
    positionFormat: function (time) {
      ...
      position -= this.settings.startTime * 2;
    ...
    isValid: function (current) {
      var maxEnd = 62 * Math.abs(this.settings.startTime - this.settings.endTime) * 2;
    ...
    export: function () {
      ...
      $('.jqs-day', $this.element).each(function (index, day) {
        ...
        data.push({
          day: index,
          weekday: $this.dateOfWeekDay($this.settings.year, $this.settings.month, index+1),
          periods: periods
        });
      });
      ...
      $.each(dataImport, function (index, data) {
        $.each(data.periods, function (index, period) {
          ...
          if ($.isArray(period)) {
            ...
            height = $this.positionFormat(period.end);
            options = period;
            options['orders'] = period.orders.length;
          }
          ...
          var status = true;
          if (!$this.add(parent, position, height - position, options, period.uuid)) {
            status = false;
          }
